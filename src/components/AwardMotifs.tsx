...
function LaurelSide({ className = "", flip = false }: MotifProps & { flip?: boolean }) {
  return (
    <svg
      aria-hidden="true"
      className={className}
    ...
    >
      <g transform={flip ? "translate(52 0) scale(-1 1)" : undefined}>
        <path d="M42 106C17 84 9 53 18 14" stroke="currentColor" strokeLinecap="round" strokeWidth="2.2" />
        {[
          [19, 22, 8, -20],
          [17, 34, 10, -34],
          [16, 48, 12, -45],
...
export function AwardSeal({ className = "" }: MotifProps) {
  return (
    <div className={["relative aspect-square text-gold-200", className].join(" ")} aria-hidden="true">
      ...
      <LaurelSide className="absolute right-1 top-1/2 h-[72%] w-[34%] -translate-y-1/2 text-current/85" flip />
...
export function StarRating({ count = 5, className = "" }: MotifProps & { count?: number }) {
  return (
    <div className={["flex gap-1.5 text-gold-200", className].join(" ")} aria-label={`${count} star rating`}>
      {Array.from({ length: count }).map((_, index) => (
        <StarIcon className="h-7 w-7 drop-shadow-[0_8px_16px_rgba(216,169,72,0.28)]" key={index} />
      ))}
    </div>
  );
}
...
export function HeroAwardPanel() {
  return (
    <div className="relative hidden min-h-[360px] overflow-hidden rounded-lg border border-gold-200/35 bg-navy-950/65 p-8 shadow-2xl backdrop-blur lg:block" aria-hidden="true">
      ...
      <div className="relative flex h-full flex-col items-center justify-center">
        ...
        <div className="mt-7 grid w-full max-w-xs grid-cols-3 gap-3">
          {[0, 1, 2].map((item) => (
            <div className="h-20 rounded-t-full border border-gold-200/25 bg-white/5" key={item} />
          ))}
        </div>
      </div>
    </div>
